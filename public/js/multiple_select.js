...
var checkbox_select = function (params) {
    ...
    if (!params.selector) {
        console.error("selector needs to be defined");
        error = true;
    }

    // If the selector is not a string
    if (typeof params.selector != "string") {
        console.error("selector needs to be a string");
        error = true;
    }

    // If the element is not a form
    if (!$(params.selector).is("form")) {
        console.error("Element needs to be a form");
        error = true;
    }

    // If the element doesn't contain a select
    if ($(params.selector).find("select").length < 1) {
        console.error("Element needs to have a select in it");
        error = true;
    }

    // If the element doesn't contain option elements
    if ($(params.selector).find("option").length < 1) {
        console.error("Select element needs to have an option in it");
        error = true;
    }

    // If the select element doesn't have a name attribute
    if ($(params.selector).find("select").attr("name") == undefined) {
        console.error("Select element needs to have a name attribute");
        error = true;
    }
    ...
    var that = this,
        ...
        updateCurrentlySelected = function () {
            ...
            if (selected.length == 0) {
                ...
                selected.length == $_ul.find("input[type=checkbox]").length
            ) {
                $_select_anchor.text(all_translation);
            } else {
                $_select_anchor.text(
                    selected.length + " " + selected_translation
                );
            }
        },
    ...
    that.onApply =
        typeof params.onApply == "function"
            ? params.onApply
            : function (e) {
    ...
    that.update = function () {
        ...
        $_native_select.find("option").each(function (i) {
            $_ul.append(
                createItem($(this).text(), $(this).val(), $(this).data("count"))
            );
        });
    ...
    $_search.keyup(function () {
        ...
        if (search.length == 1) {
            $_ul.find("label").each(function () {
                if (
                    $(this).text().toLowerCase().charAt(0) == search.charAt(0)
                ) {
            ...
            if ($_ul.text().toLowerCase().indexOf(search) == -1) {
                ...
                if (!$_not_found.hasClass("hide")) $_not_found.addClass("hide");
            }
